...
let /** {String} */ apiUrl = "https://api.github.com/users/facebook";
let /** {String} */ repoUrl,
  followerUrl,
  followingUrl = "";
// totalReposCount
let /** {Number} */ totalReposCount = 0;
...
window.updateProfile = function (profileUrl) {
  ...
  fetchData(
    ...
    (data) => {
      ...
      totalReposCount = public_repos;
      $profileCard.innerHTML = `
        <figure class="${
          type === "User" ? "avatar-circle" : "avatar-rounded"
        } img-holder" style="--width:280; --height: 280" >
            <img src="${avatar_url}" width="280" height="280" alt="${username}" class="img-cover"></figure>
        ...
        ${
          twitter_username
            ? ` <li class="meta-item">
                ...
                <a href="https://twitter.com/${twitter_username}" target="_blank" class="meta-text">@${twitter_username}</a></span>
...
const updateRepository = function (page = 1, per_page = 10) {
  // Implement pagination
  const paginationUrl = `${repoUrl}?sort=created&per_page=${per_page}&page=${page}`;

  fetchData(paginationUrl, function (data) {
    $repoPanel.innerHTML = `<h2 class="sr-only">Repositories</h2>`;
    forkedRepos = data.filter((item) => /**{Boolean} */ item.fork);

    const /**{Array} */ repositories = data;
    ...
    if (repositories.length) {
      for (const repo of repositories) {
       ...
       createPaginationLinks(page, per_page);
    } else {
      $repoPanel.innerHTML = `
...
const createPaginationLinks = function (currentPage, perPage) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    {
      ...
      $pageLink.href = "#"; // Add actual link logic
      $pageLink.textContent = i;
      $pageLink.addEventListener("click", () => updateRepository(i, perPage));
...
const updateForkRepo = function () {
  ...
  if (forkedRepos.length) {
    for (const repo of forkedRepos) {
      ...
      const /**{Node Element} */ $forkCard = document.createElement("article");
    ...
    if (data.length) {
      for (const item of data) {
        ...
        $followerCard.innerHTML = `
                    ...
                    <button class="icon-btn" onclick="updateProfile(\'${url}\')" 
